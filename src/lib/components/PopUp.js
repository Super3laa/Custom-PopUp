...
export default function PopUp(props) {
  ...
  if (props.content) {
    for (let i = 0; i < props.content.length; i++) {
      ...
      inputs.push(<Row key={i}
         content={props.content[i]}
         primaryColor={props.primaryColor}
         dataValue={props.populateDataValues ? props.dataValues[props.content[i].title]:''}
         populateDataValues={props.populateDataValues}
          />)
    }
  }
  if (props.selectors) {
...
function Row(props) {
  ...
  if (props.content.required) {
    value = InputRequired(props.primaryColor,props.dataValue);
  } else {
    value = InputForm(props.primaryColor,props.dataValue);
  }
  return (
    <React.Fragment>
      <div className="Field">
...
function InputForm(primaryColor,val) {
...
function InputRequired(primaryColor,val) {
  const [value, setValue] = React.useState(val);
